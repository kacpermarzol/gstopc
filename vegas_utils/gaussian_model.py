...
from .general_utils import strip_symmetric, build_scaling_rotation, _axis_angle_rotation, matrix_to_quaternion
...
class GaussianModel:
    ...
    def get_scaling(self):
        s3 = torch.ones(self._scaling.shape[0], 1).to(self.device) * self.eps_s3
    ...
    def get_gaussian_sizes(self):
        ...
        """
        return torch.sqrt(torch.sum(torch.pow(torch.exp(self.get_scaling * self.scaling_modifier), 2), axis=1))
    ...
    def _load_ply(self, path):
        ...
        if self.use_dff:
            self.time_func = nn.Parameter(
                torch.tensor(time_func, dtype=torch.float, device=self.device).requires_grad_(True))
        else:
            self.time_func = torch.from_numpy(time_func).to(self.device)

        self._xyz = nn.Parameter(torch.tensor(xyz, dtype=torch.float, device=self.device).requires_grad_(True))
        self._features_dc = nn.Parameter(
            torch.tensor(features_dc, dtype=torch.float, device=self.device).transpose(1, 2).contiguous().requires_grad_(
                True))
        self._features_rest = nn.Parameter(
            torch.tensor(features_extra, dtype=torch.float, device=self.device).transpose(1, 2).contiguous().requires_grad_(
                True))
        ...
        self.m = nn.Parameter(torch.tensor(m, dtype=torch.float, device=self.device).requires_grad_(True))
        self.sigma = nn.Parameter(
            torch.tensor(sigma, dtype=torch.float, device=self.device).unsqueeze(-1).requires_grad_(True))
        self._w1 = nn.Parameter(torch.tensor(w1, dtype=torch.float, device=self.device).requires_grad_(True))
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling,
                              new_rotation, new_m, new_sigma, new_w1):
        d = {"xyz": new_xyz,
